...
/****************************************************************************/
// Map radius (2..8) -> velocity (1..5). Tweak to taste.
static inline int speed_from_radius(int rad)
{
    const int MIN_R = 2, MAX_R = 8;
    const int MIN_V = 1, MAX_V = 5;
    if (rad < MIN_R) rad = MIN_R;
    if (rad > MAX_R) rad = MAX_R;
    return MIN_V + (rad - MIN_R) * (MAX_V - MIN_V) / (MAX_R - MIN_R);
}
...
void test_spaceship()
{
    ...
    const int speed = 2;        // ship move speed
    const int N = 50;           // number of stars
    const int FPS = 30;
    const int RATE = 1000 / FPS;

    int   star_x[N], star_y[N], star_rad[N], star_vel[N];
    int   star_R[N], star_G[N], star_B[N];

    // NEW: horizontal drift via sine sway (amplitude & angular speed)
    float star_phase[N], star_omega[N]; // radians & radians/frame
    int   star_amp[N];                  // pixels

    // --- initialize stars with parallax + drift ---
    for (int i = 0; i < N; ++i)
    {
        star_x[i]    = rand() % W;
        star_y[i]    = rand() % H;
        star_rad[i]  = rand() % 7 + 2;                  // 2..8 (small = far)
        star_vel[i]  = speed_from_radius(star_rad[i]);  // vertical parallax

        // optional brightness by depth (comment out if you prefer uniform)
        int base = 70 + (star_rad[i] - 2) * 185 / 6;    // 70..255
        star_R[i] = star_G[i] = star_B[i] = base;

        // horizontal sway setup:
        // amplitude 0..4 px roughly (far = tiny sway, near = bit more)
        star_amp[i]   = (star_rad[i] - 2) * 4 / 6;      // 0..4
        // phase random 0..2π
        star_phase[i] = float(rand() % 628) / 100.0f;   // ~0..6.28
        // angular speed (far = slower sway, near = slightly faster)
        star_omega[i] = 0.02f + 0.06f * (star_rad[i] - 2) / 6.0f; // ~0.02..0.08
    }
    ...
    while (1)
    {
        ...
        if (key[LEFTARROW])  rect.x -= speed;
        if (key[RIGHTARROW]) rect.x += speed;
        if (key[UPARROW])    rect.y -= speed;
        if (key[DOWNARROW])  rect.y += speed;
        ...
        // --- Move stars (vertical) & advance drift phase ---
        for (int i = 0; i < N; ++i)
        {
            star_y[i] += star_vel[i];
            star_phase[i] += star_omega[i]; // advance sway

            if (star_phase[i] > 6.283185f)  // keep phase bounded
                star_phase[i] -= 6.283185f;

            // respawn above when leaving bottom; recompute depth-tied params
            if (star_y[i] > H + star_rad[i])
            {
                star_x[i]    = rand() % W;
                star_y[i]    = - (rand() % 40 + 10);
                star_rad[i]  = rand() % 7 + 2;
                star_vel[i]  = speed_from_radius(star_rad[i]);

                int base = 70 + (star_rad[i] - 2) * 185 / 6;  // brightness by depth
                star_R[i] = star_G[i] = star_B[i] = base;

                star_amp[i]   = (star_rad[i] - 2) * 4 / 6;    // update sway amplitude
                star_phase[i] = float(rand() % 628) / 100.0f;
                star_omega[i] = 0.02f + 0.06f * (star_rad[i] - 2) / 6.0f;
            }
        }
        ...
        // background stars (apply horizontal sway at draw time)
        for (int i = 0; i < N; ++i)
        {
            int sx = star_x[i] + int(star_amp[i] * std::sin(star_phase[i]));
            // optional guard: avoid rare off-screen draws if amp pushes past edges
            if (sx < 0) sx = 0;
            if (sx > W - 1) sx = W - 1;

            surface.put_circle(sx, star_y[i], star_rad[i],
                               star_R[i], star_G[i], star_B[i]);
        }
        ...
        // --- Frame pacing ---
        const int end = getTicks();
        const int dt  = end - start;
        const int timeleft = RATE - dt;
